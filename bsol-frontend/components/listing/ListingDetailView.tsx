...
export default function ListingDetailView({ listing }: ListingDetailViewProps) {
  ...
  const { t } = useI18n();
  const strCategoryLabel = t(`categories.${listing.category}`);

  const handleDelete = async () => {
    const boolConfirmed = window.confirm(t('detail.deleteConfirm'));
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          ...
          {t('detail.backTo', { category: strCategoryLabel })}
        </Link>
        ...
        <div className="grid gap-0 md:grid-cols-[1.1fr_1fr]">
          ...
          <div className="flex flex-col gap-4 p-5 md:p-7">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-sm text-slate-400">/ {t(`period.${listing.pricePeriod}`) === `period.${listing.pricePeriod}` ? listing.pricePeriod : t(`period.${listing.pricePeriod}`)}</span>
            </div>
